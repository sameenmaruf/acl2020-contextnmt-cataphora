...
struct DecoderLayer{
	explicit DecoderLayer(DyNetModel* mod, TransformerConfig& tfc)
		:_self_attention_sublayer(mod, tfc, true)//set the future blinding to true here
	...
	{	
		...
		dynet::Expression i_mh_src_att;
		if (last_layer)
			#ifdef PRINT_SOURCE_ATTENTION
		    		i_mh_src_att = _src_attention_sublayer.build_graph(cg, i_decl, i_enc_inp, src_mask, true);// ((num_units, Ly), batch_size)
			#else
				i_mh_src_att = _src_attention_sublayer.build_graph(cg, i_decl, i_enc_inp, src_mask);// ((num_units, Ly), batch_size)
			#endif
		else
		    i_mh_src_att = _src_attention_sublayer.build_graph(cg, i_decl, i_enc_inp, src_mask);// ((num_units, Ly), batch_size)
            ...
            , vector<dynet::real>& vec_s)
    {
        ...
        auto& d = i_decl.dim();
        vec_c = dynet::as_vector(cg.get_value(dynet::pick(i_decl, (unsigned)(d[1] - 1), 1)));// contains the representation for the last column only
        ...
        vec_s = dynet::as_vector(cg.get_value(dynet::pick(i_decl, (unsigned)(d[1] - 1), 1)));// contains the representation for the last column only     
...
struct Decoder{
	...
	{		
		...
		for (auto& dec : _v_dec_layers){
			...
			nlayer += 1;
			if (nlayer == 4)
				i_dec_l_out = dec.build_graph(cg, i_src_rep, i_dec_l_out, _self_mask, _src_mask, true);// each position in the decoder can attend to all positions (up to and including the current position) in the previous layer of the decoder.
			else
            ...
            , vector<dynet::real>& vec_s)
    {
        ...
        // compute the decoder representation
        dynet::Expression i_dec_l_out = i_tgt_rep;
        for (unsigned l = 0; l < _v_dec_layers.size()-1; ++l) {
            // stacking approach
            i_dec_l_out = _v_dec_layers[l].build_graph(cg, i_src_rep, i_dec_l_out, _self_mask, _src_mask);// each position in the decoder can attend to all positions (up to and including the current position) in the previous layer of the decoder.
        }

        // for last layer
        i_dec_l_out = (_v_dec_layers.back()).build_graph(cg, i_src_rep, i_dec_l_out, _self_mask, _src_mask, vec_c, vec_s);// each position in the decoder can attend to all positions (up to and including the current position) in the previous layer of the decoder.
			...
			, vector<dynet::real>& vec_s)
	{
		...
		// for last layer
		i_dec_l_out = (_v_dec_layers.back()).build_graph(cg, i_src_rep, i_dec_l_out, _self_mask, _src_mask, vec_s);// each position in the decoder can attend to all positions (up to and including the current position) in the previous layer of the decoder.
...
public:
    ...
    dynet::Expression build_graph(dynet::ComputationGraph &cg
		...
		, ModelStats* pstats=nullptr
		, bool is_eval_on_dev=false);
    //for getting source representations
...
//for getting source representations
std::vector<std::vector<dynet::real>> TransformerModel::compute_source_rep(dynet::ComputationGraph &cg
...
WordIdSentence TransformerModel::greedy_decode(dynet::ComputationGraph& cg, const WordIdSentence &source, unsigned length_ratio)
{
	//_tfc._is_training = false;
