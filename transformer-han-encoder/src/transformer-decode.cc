...
int main(int argc, char** argv) {
	...
	opts.add_options()
		...
		//-----------------------------------------
		("test,T", value<std::string>(), "file containing testing sentences (should be only source sentence for greedy decoding)")
		...
		//-----------------------------------------
        //("context-type", value<unsigned>()->default_value(1), "to read the original source for each type of context (1: next sentence, 2: previous sentence)")
        //-----------------------------------------
		...
		, bool r2l_target /*right-to-left decoding*/)
{
    ...
    WordIdSentences sent_corpus = read_corpus(test_file, &sd, false);
    if (use_joint_vocab){
        td = sd;
    }
