...
int main(int argc, char** argv) {
	...
	opts.add_options()
		...
		//-----------------------------------------
		//("rep_type", value<unsigned>()->default_value(1), "decide which representations are to be computed (1: monolingual from encoder, 2: monolingual from decoder)")
	...
	if (stat(config_file.c_str(), &sb) == 0 && S_ISREG(sb.st_mode)) {// check existence
		...
		TRANSFORMER_RUNTIME_ASSERT("Failed to load model(s) from: " + std::string(model_path) + "!");

	use_joint_vocab = tf_model.get()->get_config()._shared_embeddings;
	...
	//set the appropriate path for saving the representations
    	std::string output_file, rep_file;
