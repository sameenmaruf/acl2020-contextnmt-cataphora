...
// ---
void run_train(transformer::TransformerContextModel &tf, const DocCorpus &train_doccor, const DocCorpus &devel_doccor,
			   vector<vector<vector<vector<dynet::real>>>> &twordrep_cor, vector<vector<vector<vector<dynet::real>>>> &dwordrep_cor,
			   dynet::Trainer*& p_sgd,
...
void eval_on_dev(transformer::TransformerContextModel &tf,
				 ...
				 vector<vector<vector<vector<dynet::real>>>> &dwordrep_cor, std::vector<std::vector<std::vector<unsigned int>>> dev_sids_docminibatch,
				 transformer::ModelStats& dstats,
...
int main(int argc, char** argv) {
	...
	opts.add_options()
		...
		("model-file,p", value<std::string>(), "name of model file without extension")
		...
		//("steps", value<unsigned>()->default_value(600000), "maximum number of steps/updates during training")
        ...
        //("warmup-steps", value<double>()->default_value(16000), "set the number of warmup steps for training (support both GPU and CPU); inversely related to minibatch size")
        //("r0", value<float>()->default_value(1.0f), "constant to adjust the magnitude of learning rate")
		...
		//-----------------------------------------
		//("context-int", value<unsigned>()->default_value(1), "decide which type of context to use (1: source, 2: target, 3: both source and target)")
		("context-type", value<unsigned>()->default_value(1), "decide which type of context to use (1: following sentence, 2: previous sentence, 3: following and previous sentence, 4: following two sentences, 5: previous two sentences, 6: following and previous 2 sentences)")
    ...
    if (!vm.count("model-file-init")) {
        ...
        if (stat(config_file.c_str(), &sb) == 0 && S_ISREG(sb.st_mode)) {// check existence
			...
			//tfc._context_int = vm["context-int"].as<unsigned>();
    ...
    //load source and target representations
	...
	// train transformer model with source context
	run_train(tf, train_doccor, devel_doccor, tsrcwordrep_corpus, dsrcwordrep_corpus,
			  p_sgd_trainer,
				 ...
				 unsigned dev_eval_mea, unsigned dev_eval_infer_algo) // batched version of eval_on_dev
{
	...
	if (dev_eval_mea == 0) // perplexity
	{
		...
		for (unsigned d = 0; d < dev_src_docminibatch.size(); ++d) {
			...
			vector<vector<vector<dynet::real>>> dwordrep_doc = dwordrep_cor[d];

			for (unsigned i = 0; i < dev_src_doc.size(); ++i) {
				ComputationGraph cg;

				auto i_xent = tf.build_graph(cg, dev_src_doc[i], dev_trg_doc[i], dwordrep_doc, dev_doc_sids[i], nullptr, true);
			   ...
			   unsigned dev_eval_mea, unsigned dev_eval_infer_algo, unsigned update_steps)
{
	...
	eval_on_dev(tf, dev_src_docminibatch, dev_trg_docminibatch, dwordrep_cor, dev_sids_docminibatch, dstats,
	...
	while (epoch < max_epochs) {
		...
		for (unsigned iter = 0; iter < dev_every_i_reports;) {
			...
			vector<vector<vector<dynet::real>>> twordrep_doc = twordrep_cor[order[id % order.size()]];
			...
			for (unsigned i = 0; i < train_src_doc.size(); ++i){
				...
				Expression i_xent = tf.build_graph(cg, train_src_doc[i], train_trg_doc[i], twordrep_doc, train_doc_sids[i], &ctstats);
		...
		if (patience > 0 && cpt >= patience)
		{
			if (RESET_IF_STUCK){
				...
				if (SWITCH_TO_ADAM){
					...
					p_sgd = new dynet::AdamTrainer(FIX_BASE_MODEL ? tf.get_context_model_parameters() : tf.get_context_model_parameters(), 0.0001f/*maybe smaller?*/);
