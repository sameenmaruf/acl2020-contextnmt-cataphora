...
int main(int argc, char** argv) {
	...
	opts.add_options()
            ...
            ("context-type", value<unsigned>()->default_value(3), "decide which type of context to use (0: decode without context, 1: following sentence, 2: previous sentence, 3: following and previous sentence, 4: following two sentences, 5: previous two sentences, 6: following and previous 2 sentences)")
		    //-----------------------------------------
	...
	if (stat(config_file.c_str(), &sb) == 0 && S_ISREG(sb.st_mode)){// check existence
		...
		// load models
		if (!load_model_config(config_file, v_tf_models, sd, td, sm, vm["context-type"].as<unsigned>()))
		...
		, unsigned context_type)
{
	...
	while (getline(inpf, line)){
		...
		tfc._context_type = context_type;
		
		cout << "Decoding with context type: " << tfc._context_type << endl;
		...
		, bool r2l_target /*right-to-left decoding*/)
{
	...
	vector<vector<vector<vector<dynet::real>>>> twordrep_cor;
	for (unsigned sd = 0; sd < doc_corpus.size(); sd++) {
		...
		vector<vector<dynet::real>> wordrep;
		for (unsigned dl = 0; dl < sdlen; ++dl) {
			ComputationGraph cg;
			WordIdSentence source = doc_corpus[sd].at(dl);
			wordrep = v_models[0].get()->compute_source_rep(cg, source);
			
			wordrep_doc.push_back(wordrep);
		}
		twordrep_cor.push_back(wordrep_doc);
	}
	...
	for (unsigned i = 0; i < doc_corpus.size(); ++i) {
		...
		vector<vector<vector<dynet::real>>> twordrep_doc = twordrep_cor[i];
		...
		for (unsigned dl = 0; dl < vssent.size(); ++dl) {
			...
			WordIdSentence source = vssent[dl];
			sids.push_back(dl);
			WordIdSentence target = v_models[0].get()->greedy_decode(cg, source, twordrep_doc, sids, length_ratio);
