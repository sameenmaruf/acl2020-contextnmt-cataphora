...
void run_train(transformer::TransformerModel &tf, const WordIdCorpus &train_cor, const WordIdCorpus &devel_cor, 
	...
	unsigned max_epochs, unsigned patience, /*unsigned max_steps,*/
	unsigned lr_epochs, float lr_eta_decay, unsigned lr_patience,
    /*double warmup_steps,  float r0,*/
...
int main(int argc, char** argv) {
	...
	run_train(tf
		...
		, p_sgd_trainer
		, model_path
		, vm["epochs"].as<unsigned>(), vm["patience"].as<unsigned>()/*, vm["steps"].as<unsigned>() early stopping*/
		, lr_epochs, vm["lr-eta-decay"].as<float>(), lr_patience/*learning rate scheduler*/
        /*, vm["warmup-steps"].as<double>(), vm["r0"].as<float>()*/
