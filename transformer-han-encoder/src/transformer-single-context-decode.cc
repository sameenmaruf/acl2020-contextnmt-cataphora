...
int main(int argc, char** argv) {
	...
	opts.add_options()
			...
			//-----------------------------------------
			("test,T", value<std::string>(), "file containing testing sentences (should be current_src ||| next_src or prev_src ||| current_src for greedy decoding)")
			...
			("context-type", value<unsigned>()->default_value(1), "decide which type of context to use (0: decode without context (input file format: current_src ||| next_src), 1: next sentence, 2: previous sentence)")
		    //-----------------------------------------
	...
	if (vm["beam"].as<unsigned>() == 1) {
		if (vm["context-type"].as<unsigned>() == 0) {
			greedy(test_input_file, v_tf_models, vm["length-ratio"].as<unsigned>(), vm.count("remove-unk"),
				   vm.count("r2l-target"));
		}
		else {
			greedy_monolingual(test_input_file, v_tf_models, vm["length-ratio"].as<unsigned>(),
					   vm.count("remove-unk"), vm.count("r2l-target"), vm["context-type"].as<unsigned>());
		}
	}
	else
		...
		, bool r2l_target /*right-to-left decoding*/)
{
	...
	WordIdCorpus sent_corpus = read_corpus(test_file, &sd, &sd, false);
	if (use_joint_vocab){
		td = sd;
	}
	...
	unsigned int lno = 0;
	for (unsigned i = 0; i < sent_corpus.size(); ++i) {
		ComputationGraph cg;// dynamic computation graph
        WordIdSentence source = get<0>(sent_corpus[i]);
		...
		, unsigned context_type)
{
	...
	vector<vector<vector<dynet::real>>> twordrep_cor;
	for (unsigned i = 0; i < sent_corpus.size(); i++) {
		vector<vector<dynet::real>> wordrep_sent;

		ComputationGraph cg;
		WordIdSentence context_source;
		if (context_type == 1) {
			context_source = get<1>(sent_corpus[i]);
		}
		else if (context_type == 2){
			context_source = get<0>(sent_corpus[i]);
		}

        wordrep_sent = v_models[0].get()->compute_source_rep(cg, context_source);
        twordrep_cor.push_back(wordrep_sent);
	}
	cerr << "Computed representations of " << sent_corpus.size() << " sentences in context!" << endl;
	...
	for (unsigned i = 0; i < sent_corpus.size(); ++i) {
		...
		if (context_type == 1) {
			source = get<0>(sent_corpus[i]);
		}
		else if (context_type == 2){
			source = get<1>(sent_corpus[i]);
		}
		wordrep_sent = twordrep_cor[i];
		WordIdSentence target = v_models[0].get()->greedy_decode(cg, source, wordrep_sent, length_ratio);
	...
	double elapsed = timer_dec.elapsed();
	cerr << "Greedy decoding for Transformer with Monolingual Context is finished!" << endl;
	cerr << "Decoded " << lno << " sentences, completed in " << elapsed/1000 << "(s)" << endl;
}
//---